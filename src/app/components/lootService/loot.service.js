(function() {
  ...
  function lootService(enemiesService) {
    ...
    svc.gimmeTheLoot = gimmeTheLoot;
    svc.pullFromVault = pullFromVault;
    ...
    function gimmeTheLoot() {
      ...
      if (chance > 99910) {
        loot.push(angular.copy(svc.epics[Math.floor(Math.random()*svc.epics.length)]));
        loot.push(angular.copy(svc.rares[Math.floor(Math.random()*svc.rares.length)]));
        ...
        loot.push(angular.copy(svc.rares[Math.floor(Math.random()*svc.rares.length)]));
        loot.push(angular.copy(svc.uncommons[Math.floor(Math.random()*svc.uncommons.length)]));
      } else if (chance > 90000) {
        loot.push(angular.copy(svc.uncommons[Math.floor(Math.random()*svc.uncommons.length)]));
      } else if (chance > 70000) {
        loot.push(angular.copy(svc.commons[Math.floor(Math.random()*svc.commons.length)]));
        loot.push(angular.copy(svc.commons[Math.floor(Math.random()*svc.commons.length)]));
      ...
      angular.forEach(enemiesService.getCurrentEnemies(), function(enemy) {
        angular.forEach(enemy.loot, function(enemyLoot) {
          loot.push(angular.copy(enemyLoot));
        });
      });
