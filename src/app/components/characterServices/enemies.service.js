(function () {
    ...
    enemiesService.$inject = ["progressTracker", "boardManager", "gameStatsService"];

    function enemiesService(progressTracker, boardManager, gameStatsService) {
        var svc = this;
        ...
        function deliverRegularDamage(enemy, power, ally) {
            var damage = Math.round(((1.7 + ((Math.random() * 6) / 10)) * power) - (.75 * enemy.stats.defense));
            if (damage <= 0) {
                damage = 0;
            }
            gameStatsService.allyDeliveredDamage(damage, ally);
            enemy.stats.health -= damage;
        }
